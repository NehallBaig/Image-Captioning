...
def test():
    ...
    image_data = bytes(content['img'], encoding="ascii")
    with open('static/file.jpg', 'wb') as fh:
        fh.write(base64.decodebytes(image_data))

    # with open("test.txt", "w") as fo:
    #     fo.write(content['img'])
...
def after():
    ...
    global model, resnet, vocab, inv_vocab, cap
    content = request.get_json()
...
if __name__ == "__main__":
    app.run(host='192.168.2.115', port=5002, debug=True)
